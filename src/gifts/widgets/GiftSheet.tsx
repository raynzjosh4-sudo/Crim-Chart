...
import { View, Text, Image, TouchableOpacity, StyleSheet, Modal, ActivityIndicator, FlatList } from 'react-native';
...
export const GiftSheet: React.FC<GiftSheetProps> = ({
  ...
  const [gifts] = useState<GiftModel[]>([{
    ...
    name: 'Crown',
    coinPrice: 500,
    imageUrl: 'https://cdn-icons-png.flaticon.com/512/694/694982.png',
    isAnimated: true
  }]);
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={() => onClose()}>
      <View style={styles.backdrop}>
        <View style={styles.sheet}>
          ...
          {gifts.length === 0 ? <GiftGridSkeleton count={8} /> : <FlatList data={gifts} keyExtractor={g => g.id} renderItem={({
          item
            ...
            uri: item.imageUrl
                ...
                <TouchableOpacity activeOpacity={1} style={[styles.sendBtn, {
            backgroundColor: themeColor
          }]} onPress={() => onClose(item)}>
                  <Text style={styles.sendText}>SEND</Text>
                </TouchableOpacity>
              </View>} ItemSeparatorComponent={() => <View style={styles.separator} />} contentContainerStyle={{
          paddingVertical: 16
        }} />}
        </View>
      </View>
...
const TabBtn: React.FC<{
  ...
  onSelect,
  color
}) => <TouchableOpacity activeOpacity={1} style={[styles.tab, active && {
