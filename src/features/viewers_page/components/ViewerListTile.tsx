  ...
  return <TouchableOpacity activeOpacity={0.7} onPress={onPress} style={[styles.container, {
    borderBottomColor: colors.border
      ...
      <View style={styles.avatarContainer}>
        <UserAvatar userId={userId} name={name} fallbackUrl={avatarUrl} size={50} forceOnline={isOnline} forceHasStatus={hasStatus} forceStatusCount={statusSegmentCount} />
      </View>
      ...
      <View style={styles.infoContainer}>
        ...
        {viewedStatuses && viewedStatuses.length > 0 ? <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{
        marginTop: 4
      }}>
          ...
          return <View key={status.id || idx} style={styles.statusThumbContainer}>
                  {thumbUrl ? <Image source={{
              uri: thumbUrl
            }} style={styles.statusThumb} /> : <View style={[styles.statusThumb, {
              backgroundColor: '#333',
              justifyContent: 'center',
              alignItems: 'center'
            }]}>
          ...
          </ScrollView> : <Text style={[styles.viewedAtText, {
        color: colors.text
      }]} numberOfLines={1}>
            Viewed {viewedAtText}
          </Text>}
