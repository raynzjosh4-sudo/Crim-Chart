...
export const ViewersPage = () => {
  ...
  const fetchViewers = async (pageNumber: number) => {
    ...
    try {
      ...
      if (pageNumber === 0) {
        const { data: countData } = await supabase.rpc('get_unique_status_viewers_count', {
          p_author_id: user.id
        });
        ...
        supabase.from('profiles').update({ active_status_views_count: 0 }).eq('id', user.id).then(({ error }) => {
          if (error) console.log('Error resetting badge:', error);
          else {
      ...
      const { data, error } = await supabase.rpc('get_unique_status_viewers', {
        p_author_id: user.id,
        p_limit: PAGE_SIZE,
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        <View style={styles.header}>
          <Text style={[styles.headerText, { color: colors.text }]}>Status Viewers</Text>
        </View>
        <ViewersPageShimmer />
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <FlatList
        data={viewers}
        keyExtractor={(item, index) => item.id || `${item.status_id}_${item.viewer_id}_${index}`}
        renderItem={({ item }) => (
          <ViewerListTile
            userId={item.viewer_id}
            name={item.display_name || 'Unknown'}
            avatarUrl={item.profile_image_url || null}
            viewedAtText={item.last_viewed_at ? formatTimeAgo(item.last_viewed_at) : 'recently'}
            viewedStatuses={item.viewed_statuses}
            onPress={() => console.log(`Tapped on viewer: ${item.display_name}`)}
          />
        )}
        ListHeaderComponent={() => (
          <View style={styles.header}>
            <Text style={[styles.headerText, { color: colors.text }]}>Status Viewers</Text>
            <Text style={[styles.subHeaderText, { color: colors.text }]}>{totalViews} views</Text>
          </View>
        )}
        ...
        ListEmptyComponent={() => (
          <View style={{ padding: 20, alignItems: 'center' }}>
            <Text style={{ color: colors.text, opacity: 0.5 }}>No one has viewed your status yet.</Text>
          </View>
        )}
      />
    </SafeAreaView>
