...
export const VideoTabView: React.FC<VideoTabViewProps> = ({
  ...
  } = useChannelMoments(user?.id || '', channelId);
  const {
    posts,
  ...
  const videos = posts.filter((p: any) => p.thumbnailUrl || p.videoUrl || p.imageUrls?.length > 0).map((p: any) => videoPostFromMap({
    id: p.id,
    post_id: p.id,
  ...
  return <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <VideoPromotionBanner channelName={channelName} channelTitle={channelTitle} onPostMoment={() => {
        if (Platform.OS === 'web') {
        ...
        {postsLoading ? <ActivityIndicator color="#FACD11" style={{
        marginTop: 32
      }} /> : videos.length === 0 ? <View style={styles.noVideosContainer}>
            ...
            return <View key={video.id} style={styles.cardWrapper}>
                    <VideoCard video={video} height={HEIGHTS[globalIndex % HEIGHTS.length]} index={globalIndex} allVideos={videos} />
                  </View>;
          })}
            </View>
            <View style={styles.column}>
