...
export const MovieBoxDetailPage: React.FC<MovieBoxDetailPageProps> = ({ id, onClose }) => {
    ...
    <View style={styles.headerContainer}>
      ...
      <View style={styles.contributorsSection}>
        <RecentContributorsWidget
          ...
          onSelectMember={setSelectedFilterUserId}
          onLongPressMember={setViewerMemberId}
          onAddPress={() => {
            if (isDesktop) setShowPostingModal(true);
            else router.push(`/movie-box/post/${id}`);
          }}
        />
      </View>
    </View>
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      {selectedVideoParams && (
        <Modal visible={true} animationType="slide" onRequestClose={() => setSelectedVideoParams(null)}>
          <VideoFeedPage
            initialVideos={displayedVideos.filter(t => t.isShort).map(t => ({
              id: t.id,
              postId: t.id,
            ...
            initialIndex={Math.max(0, displayedVideos.filter(t => t.isShort).findIndex(t => t.id === selectedVideoParams.id))}
            showBack={true}
