...
export const SelectUsersBottomSheet: React.FC<SelectUsersBottomSheetProps> = ({
  ...
  const pan = useRef(new Animated.ValueXY({ x: 0, y: OFFSET })).current;
  ...
  const renderItem = useCallback(({ item: user }: { item: UserData }) => {
    return (
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ width: '100%', alignItems: 'center' }}>
          <Animated.View style={[styles.container, { transform: [{ translateY: pan.y }] }]}>
            ...
            <FlatList
              ...
              style={styles.scroll}
              contentContainerStyle={{ paddingBottom: Math.max(40, insets.bottom + 20), paddingHorizontal: 20 }}
              ListEmptyComponent={
                loading ? <UserListSkeleton count={5} /> : <Text style={styles.emptyText}>No users found</Text>
              }
            />
          </Animated.View>
...
const styles = StyleSheet.create({
  ...
  container: { width: '100%', height: SHEET_MAX_HEIGHT, backgroundColor: '#000000', borderTopLeftRadius: 24, borderTopRightRadius: 24 },
  dragHandle: { width: 36, height: 4, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 2, alignSelf: 'center', marginTop: 12, marginBottom: 8 },
  ...
  searchContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#111111', marginHorizontal: 20, marginBottom: 20, borderRadius: 12, paddingHorizontal: 16, height: 48 },
  searchInput: { flex: 1, marginLeft: 12, color: '#FFF', fontSize: 16 },
  scroll: { width: '100%' },
  emptyText: { color: 'rgba(255,255,255,0.4)', textAlign: 'center', marginTop: 32, fontSize: 15 },
