...
import { Modal, View, Text, TouchableOpacity, FlatList, StyleSheet, ActivityIndicator, Animated, Platform, useWindowDimensions } from 'react-native';
...
export const TagOverlay: React.FC<TagOverlayProps> = ({
  ...
  if (isDesktop) {
    return <DesktopTagOverlay visible={visible} onClose={onClose} postId={postId} sourceChannelId={sourceChannelId} linkChain={linkChain} />;
  }
  ...
  const loadChannels = useCallback(async (reset = false) => {
    ...
    try {
      ...
      const data = await fetchUserTagChannels({
        limit: 20,
        offset: page * 20
      });
      if (reset) {
  ...
  const renderBody = () => {
    ...
    return <TagCarousel cards={channels.map(c => ({
      id: c.id,
      title: c.name,
      description: c.description,
      imageUrl: c.avatarUrl
    }))} isLoadingMore={isLoadingMore} onEndReached={() => loadChannels(false)} postId={postId} sourceChannelId={sourceChannelId} linkChain={linkChain} onTagSuccess={onClose} />;
  };
  return <Modal visible={visible} transparent animationType="none" onRequestClose={onClose}>
      <TouchableOpacity style={styles.backdrop} activeOpacity={1} onPress={onClose}>
        ...
        opacity: opacityAnim,
        backgroundColor: dark ? '#121212' : colors.card
