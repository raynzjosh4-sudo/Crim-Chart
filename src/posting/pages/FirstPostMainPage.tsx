...
import { View, Text, StyleSheet, TouchableOpacity, FlatList, ActivityIndicator, Dimensions, SafeAreaView } from 'react-native';
...
export function FirstPostMainPage() {
  ...
  const loadAssets = useCallback(async (cursor?: string, reset = false) => {
  ...
  return <SafeAreaView style={styles.container}>
      <View style={styles.appBar}>
        ...
        <View style={styles.actions}>
          {selectedItems.size > 0 && <TouchableOpacity activeOpacity={1} onPress={onNext}>
      ...
      <View style={styles.tabBar}>
        <TouchableOpacity activeOpacity={1} style={[styles.tab, activeTab === 'photos' && styles.activeTab]} onPress={() => setActiveTab('photos')}>
          <Text style={[styles.tabText, activeTab === 'photos' && styles.activeTabText]}>
            {tr('photos_tab') || 'Photos'}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity activeOpacity={1} style={[styles.tab, activeTab === 'videos' && styles.activeTab]} onPress={() => setActiveTab('videos')}>
      ...
      {!hasPermission ? <View style={styles.center}>
          <Text style={{
        color: 'rgba(255,255,255,0.5)'
      }}>Please grant gallery access via Settings.</Text>
        </View> : isLoading ? <View style={styles.center}>
          <ActivityIndicator color={colors.primary} />
        </View> : <FlatList data={assets} numColumns={3} keyExtractor={item => item.id} contentContainerStyle={styles.gridContent} onEndReached={handleEndReached} onEndReachedThreshold={0.4} ListFooterComponent={isLoadingMore ? <ActivityIndicator color={colors.primary} style={{
      padding: 16
    }} /> : null} renderItem={({
      item
    }) => {
      const isSelected = selectedItems.has(item.id);
      return <TouchableOpacity activeOpacity={1} style={[styles.gridItem, isSelected && {
        borderColor: colors.primary,
        borderWidth: 3
      }]} onPress={() => toggleSelection(item)} activeOpacity={0.8}>
                <ExpoImage source={{
          uri: item.uri
        }} contentFit="cover" style={StyleSheet.absoluteFill} />
                {item.mediaType === MediaLibrary.MediaType.video && <View style={styles.videoOverlay}>
