...
export default function ConnectionPrivacyPage() {
  ...
  const updatePreference = async (key: string, value: boolean) => {
    ...
    try {
      ...
      if (!currentStats) currentStats = { user_id: user.id };
      
      const updatedStats = { ...currentStats, [key]: value };
  ...
  const renderToggle = (title: string, description: string, value: boolean, onValueChange: (v: boolean) => void) => (
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {renderToggle(
          'Status Text',
          'Display your exact status label (e.g. "Highly Selective") in the stats box on your profile.',
          showStatusText,
          (val) => updatePreference('show_status_text', val)
        )}

        {renderToggle(
          'Country Preferences',
          'Show which countries you connect with the most on your profile.',
          showCountryPref,
          (val) => updatePreference('show_country_pref', val)
        )}

        {renderToggle(
          'Age Preferences',
          'Show the age ranges you most frequently connect with.',
          showAgePref,
          (val) => updatePreference('show_age_pref', val)
        )}
        ...
        <View style={styles.guideContainer}>
          ...
          <View style={styles.guideRow}>
            <View style={[styles.guideRing, { borderColor: '#10B981' }]} />
            <View style={styles.guideTextContainer}>
              <Text style={[styles.guideColorName, { color: '#10B981' }]}>Highly Selective</Text>
              <Text style={styles.guideDesc}>Very picky and focuses energy on a few select people.</Text>
            </View>
          </View>

          <View style={styles.guideRow}>
            <View style={[styles.guideRing, { borderColor: '#3B82F6' }]} />
            <View style={styles.guideTextContainer}>
              <Text style={[styles.guideColorName, { color: '#3B82F6' }]}>Active</Text>
              <Text style={styles.guideDesc}>Actively dating but keeps their circle relatively manageable.</Text>
            </View>
          </View>

          <View style={styles.guideRow}>
            <View style={[styles.guideRing, { borderColor: '#F59E0B' }]} />
            <View style={styles.guideTextContainer}>
              <Text style={[styles.guideColorName, { color: '#F59E0B' }]}>Exploring</Text>
              <Text style={styles.guideDesc}>Talking to many people and openly exploring options.</Text>
            </View>
          </View>

          <View style={styles.guideRow}>
            <View style={[styles.guideRing, { borderColor: '#EF4444' }]} />
            <View style={styles.guideTextContainer}>
              <Text style={[styles.guideColorName, { color: '#EF4444' }]}>Casting a Wide Net</Text>
