...
export const PersonalInformationPage = ({
  ...
  const handleSave = async () => {
    ...
    if (success) {
      ...
      if (isModal && onClose) onClose();else router.back();
    } else {
  ...
  return <View style={[styles.container, isModal && isDesktop && styles.desktopContainer]}>
      <View style={[isModal && isDesktop ? styles.desktopModal : styles.pageWrapper]}>
      <ChartAppBar title="Personal Information" showBack={false} useSafeArea={!isModal} showBorder={!isModal} leading={<CrimchartBackButton onPress={handleBack} color={colors.text} />} actions={[<TouchableOpacity activeOpacity={1} key="save" onPress={handleSave} disabled={isLoading} style={styles.saveButton}>
            <Text style={[styles.saveText, isLoading && {
          opacity: 0.5
        }]}>Save</Text>
          </TouchableOpacity>]} />
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {showDatePicker && <DateTimePicker value={birthday || new Date(1995, 0, 1)} mode="date" display="spinner" maximumDate={new Date()} onChange={handleDateChange} />}
        ...
        <View style={styles.inputFieldContainer}>
          <TextInput style={[styles.blockInput, Platform.OS === 'web' && {
            outlineStyle: 'none'
          } as any]} value={crownTitle} onChangeText={setCrownTitle} placeholder="Enter your crown title..." placeholderTextColor="rgba(255,255,255,0.3)" />
        </View>
        ...
        <View style={styles.inputFieldContainer}>
          <TextInput style={[styles.blockInput, {
            ...
            outlineStyle: 'none'
          } as any]} value={bio} onChangeText={setBio} placeholder="Tell us about yourself..." placeholderTextColor="rgba(255,255,255,0.3)" multiline />
        </View>
      ...
      <Modal visible={showGenderModal} transparent animationType="slide">
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowGenderModal(false)}>
          <View style={[styles.modalContent, {
            ...
            {['Male', 'Female', 'Non-binary', 'Prefer not to say'].map(g => <TouchableOpacity activeOpacity={1} key={g} style={styles.modalItem} onPress={() => selectGender(g)}>
