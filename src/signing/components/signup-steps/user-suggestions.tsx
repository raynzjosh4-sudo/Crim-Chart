...
export default function UserSuggestionsPage({ onNext, onBack, onClose }: StepProps) {
  ...
  return <SafeAreaView style={styles.container}>
      {!isDesktop && <ChartAppBar title="" showBorder isLoading={isLoading} actions={[<TouchableOpacity activeOpacity={1} key="skip" onPress={handleFinish} disabled={isLoading}>
      ...
      <View style={styles.flexOne}>
        <View style={styles.content}>
          ...
          <Text style={[styles.subtitle, isDesktop && {
          textAlign: 'center'
        }]}>
          ...
          {usersLoading ? <ActivityShimmer /> : users.length === 0 ? <View style={styles.centerBox}>
              <Text style={styles.subtitle}>No suggestions found right now.</Text>
            </View> : <FlatList data={users} keyExtractor={item => String(item.id)} contentContainerStyle={styles.listPadding} renderItem={({
          item
        }) => <View style={styles.userItem}>
                  {item.profileImageUrl ? <Image source={{
            uri: item.profileImageUrl
          }} style={styles.userIcon} /> : <View style={[styles.userIcon, styles.placeholderIcon]}>
                  ...
                  <View style={styles.buttonContainer}>
                    <FollowUserButton targetUserId={item.id} size="small" onToggle={handleToggle} />
                  </View>
                </View>} />}

          <View style={styles.footer}>
            <TouchableOpacity activeOpacity={1} style={[styles.finishButton, followCount < 2 && styles.finishButtonDisabled]} onPress={handleFinish} disabled={followCount < 2}>
