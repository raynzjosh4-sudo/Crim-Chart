...
export default function BirthdayPage({ onNext, onBack, onClose }: StepProps) {
  ...
  const handleNext = async () => {
    ...
    if (validDate === 'too_young') {
      ChartToast.showError(null, {
        ...
        message: 'You must be at least 13 years old to use CrimChart.'
      });
      return;
    }
    setIsLoading(true);
      ...
      <View style={styles.flexOne}>
        ...
        <View style={styles.datePickerContainer}>
          <View style={styles.dateInputRow}>
            <TextInput ref={monthRef} style={[styles.dateInput, Platform.OS === 'web' ? {
              outlineStyle: 'none'
            } as any : {}]} placeholder="MM" placeholderTextColor="rgba(255, 255, 255, 0.2)" keyboardType="number-pad" maxLength={2} value={month} onChangeText={text => {
              const clean = text.replace(/[^0-9]/g, '');
              setMonth(clean);
              if (clean.length === 2) dayRef.current?.focus();
            }} />
            <Text style={styles.dateSeparator}>/</Text>
            <TextInput ref={dayRef} style={[styles.dateInput, Platform.OS === 'web' ? {
              outlineStyle: 'none'
            } as any : {}]} placeholder="DD" placeholderTextColor="rgba(255, 255, 255, 0.2)" keyboardType="number-pad" maxLength={2} value={day} onChangeText={text => {
              const clean = text.replace(/[^0-9]/g, '');
              setDay(clean);
            ...
            <Text style={styles.dateSeparator}>/</Text>
            <TextInput ref={yearRef} style={[styles.dateInput, styles.yearInput, Platform.OS === 'web' ? {
              outlineStyle: 'none'
            } as any : {}]} placeholder="YYYY" placeholderTextColor="rgba(255, 255, 255, 0.2)" keyboardType="number-pad" maxLength={4} value={year} onChangeText={text => {
              const clean = text.replace(/[^0-9]/g, '');
              setYear(clean);
        ...
        <View style={styles.genderRow}>
          {genders.map(gender => {
            ...
            return <TouchableOpacity activeOpacity={1} key={gender.id} style={[styles.genderItem, isSelected && styles.genderItemSelected]} onPress={() => setSelectedGender(gender.id)}>
                <Icon size={20} color={isSelected ? colors.primary : 'rgba(255, 255, 255, 0.5)'} />
                <Text style={[styles.genderLabel, isSelected && styles.genderLabelSelected]}>{gender.label}</Text>
              </TouchableOpacity>;
          })}
        </View>
        ...
        <TouchableOpacity activeOpacity={1} style={[styles.nextButton, (!selectedGender || month.length < 1 || day.length < 1 || year.length < 4) && styles.nextButtonDisabled]} onPress={handleNext} disabled={!selectedGender || month.length < 1 || day.length < 1 || year.length < 4 || isLoading}>
