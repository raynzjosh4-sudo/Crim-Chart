...
export default function GoogleSignInPage() {
  ...
  return <View style={[styles.container, {
    backgroundColor: colors.background
        ...
        }} style={styles.googleLogo} resizeMode="contain" />
          ...
          color: colors.text
            ...
            color: colors.text
                ...
                <View style={styles.listContainer}>
                    {loading ? <ActivityIndicator size="large" color={colors.primary} /> : emails.length === 0 ? <Text style={[styles.errorText, {
          color: 'rgba(255, 255, 255, 0.7)'
        }]}>
                            {error || tr('google_no_accounts')}
                        </Text> : <FlatList data={emails} keyExtractor={item => item} renderItem={({
          item
        }) => {
          const initials = item.length > 0 ? item[0].toUpperCase() : 'A';
          const name = item.split('@')[0];
          return <View>
                                        <TouchableOpacity activeOpacity={1} style={styles.accountTile} onPress={() => handleSelectEmail(item)}>
