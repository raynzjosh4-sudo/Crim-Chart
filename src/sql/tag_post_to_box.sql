...
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER -- Runs with privileges of the creator so it bypasses RLS if needed, but we can enforce logic inside
AS $$
DECLARE
  v_box_item_id UUID;
  v_box_owner_id UUID;
...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO public.box_items (box_id, post_id)
    VALUES (p_box_id, p_post_id)
