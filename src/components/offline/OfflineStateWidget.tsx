...
export const OfflineStateWidget: React.FC<OfflineStateWidgetProps> = ({
  onRetry,
  overrideState
}) => {
  ...
  if (overrideState) {
    ...
    } else if (netInfo.type === 'cellular' && (netInfo.details?.cellularGeneration === '2g' || netInfo.details?.cellularGeneration === '3g')) {
      currentState = 'slow';
  ...
  const title = isOffline ? t('offline.no_internet_title', 'No Internet Connection') : t('offline.slow_connection_title', 'Slow Connection');
  const subtitle = isOffline ? t('offline.no_internet_subtitle', "It looks like you're offline. Please check your network connection and try again.") : t('offline.slow_connection_subtitle', "Your internet connection seems to be slow. Some content may take longer to load or fail.");
  return <Modal transparent={true} visible={true} animationType="fade">
      ...
      backgroundColor: colors.background
        ...
        borderColor: colors.border
          ...
          {!isOffline && <TouchableOpacity style={styles.closeButton} onPress={() => setDismissed(true)} hitSlop={{
          top: 10,
          bottom: 10,
          left: 10,
          right: 10
          ...
          {isDesktop ? <View style={styles.contentDesktop}>
              <Text style={[styles.title, styles.titleDesktop, {
            color: colors.text
          }]}>{title}</Text>
              <Text style={[styles.subtitle, styles.subtitleDesktop, {
            color: 'rgba(255,255,255,0.6)'
          }]}>{subtitle}</Text>
              {onRetry && <TouchableOpacity style={[styles.retryButton, styles.retryButtonDesktop]} onPress={onRetry} activeOpacity={0.8}>
            ...
            color: 'rgba(255,255,255,0.6)'
              ...
              {onRetry && <TouchableOpacity style={styles.retryButton} onPress={onRetry} activeOpacity={0.8}>
