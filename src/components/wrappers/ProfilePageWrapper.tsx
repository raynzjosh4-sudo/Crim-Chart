  ...
  const renderActions = () => {
    if (isCurrentUser) {
      return <View style={styles.actionRow}>
          ...
          <TouchableOpacity onPress={() => router.push('/inbox')} style={[styles.actionBtn, styles.secondaryBtn]} activeOpacity={0.85}>
    ...
    return <View style={styles.actionRow}>
        ...
        <InboxUserButton targetUserId={activeUserId!} targetUserName={userData?.displayName || userData?.username || ''} targetUserAvatar={userData?.profileImageUrl || undefined} style={[styles.actionBtn, styles.secondaryBtn]} textStyle={styles.secondaryBtnText} />
      </View>;
  };
  return <>
      <ProfilePage userId={activeUserId} userData={userData || undefined} customActions={renderActions()} isLoading={isLoading} onEditProfile={() => {
      const isDesktop = Platform.OS === 'web' && Dimensions.get('window').width >= 768;
      if (isDesktop) {
        setIsEditProfileOpen(true);
      } else {
      ...
      {connectionsPanel && <FollowersListPage targetUserId={connectionsPanel.userId} initialTab={connectionsPanel.tab} isPanel={true} onClose={() => setConnectionsPanel(null)} />}
    </>;
};