    ...
    <>
      <View style={styles.diskContainer}>
        <View style={styles.diskWrapper}>
          <AnimatedDisk
            isPlaying={isPlaying}
            onPress={selectedAudio ? togglePlayback : () => {}}
            size={260}
            imageUrl={diskImageUrl}
            placeholderText={t('tap_to_add_audio' as any) || "Tap to add audio"}
          />
          {/* Cover image button — always visible: pick if no audio, pick if has audio (allows override) */}
          {!selectedAudio && (
            <TouchableOpacity
              activeOpacity={0.8}
              style={[styles.addCoverButton, { backgroundColor: theme.colors.surface }]}
          ...
          {selectedAudio && (
            <>
              ...
              {onRemoveAudio && (
                <TouchableOpacity
                  activeOpacity={0.8}
                  style={[styles.addCoverButton, { backgroundColor: theme.colors.surface, top: 'auto', bottom: 10, right: 'auto', left: 10 } as any]}
                  onPress={() => { setCustomThumbnail(null); onRemoveAudio(); }}
        ...
        <View style={styles.metadataFields}>
          <TextInput
            style={[styles.metaInput, { color: theme.colors.text, borderBottomColor: 'rgba(255,255,255,0.2)' }, Platform.OS === 'web' && ({ outlineStyle: 'none' } as any)]}
            placeholder={t('track_title' as any) || "Track Title"}
          ...
          <TextInput
            style={[styles.metaInput, { color: theme.colors.text, borderBottomColor: 'rgba(255,255,255,0.2)' }, Platform.OS === 'web' && ({ outlineStyle: 'none' } as any)]}
            placeholder={t('artist_name' as any) || "Artist Name"}
            placeholderTextColor={theme.colors.text + '80'}
            value={artist}
          ...
          >
            <Text style={[styles.addLyricsText, { color: theme.colors.primary }]}>
      ...
      <ComposerLyricsSheet
        visible={isLyricsSheetOpen}
        onClose={() => setIsLyricsSheetOpen(false)}
