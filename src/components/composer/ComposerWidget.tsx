...
import { usePostingStore, MediaType, PostType } from '@/core/store/usePostingStore';

export const ComposerWidget = ({ onPostSuccess }: { onPostSuccess?: () => void }) => {
  ...
  const handlePost = async () => {
    ...
    const success = await usePostingStore.getState().createPost({
      media,
      caption: text,
      postType: 'post', // Default regular post
      isPublicFeed: true,
      allowComments: true,
  ...
  const renderInputArea = () => (
    <View style={styles.inputArea}>
      <UserAvatar
        userId={user?.id || ''}
        fallbackUrl={user?.profileImageUrl}
        name={user?.displayName}
        size={40}
      />
      <TextInput
        style={[styles.input, { color: theme.colors.text }, Platform.OS === 'web' && ({ outlineStyle: 'none' } as any)]}
        placeholder={t('what_is_happening' as any) || "What's happening?"}
  ...
  return (
    <View style={styles.container}>
      {isPickingMusic ? (
        <View style={{ flex: 1 }}>
          <LocalMusicList
            ...
            prefetchedAssets={prefetchedAssets ?? undefined}
      ...
      {showCategories && (
        <>
          <Pressable
            style={[StyleSheet.absoluteFillObject, { zIndex: 9998, backgroundColor: 'rgba(0,0,0,0.5)', cursor: 'default' as any }]}
            onPress={() => setShowCategories(false)}
          />
          <View style={[
            styles.musicDetailsSheet, 
      ...
      <View style={[styles.footer, { borderTopColor: 'rgba(255,255,255,0.1)' }]}>
        <View style={styles.toolbar}>
          ...
          <TouchableOpacity 
            activeOpacity={0.8} 
            style={[styles.categoryChip, { backgroundColor: category ? theme.colors.primary + '33' : 'rgba(255,255,255,0.1)' }]}
            onPress={() => setShowCategories(true)}
          >
            <Text style={{ color: category ? theme.colors.primary : '#FFF', fontSize: 12, fontWeight: 'bold' }}>
        ...
        <TouchableOpacity
          activeOpacity={0.8}
          style={[styles.postButton, { backgroundColor: theme.colors.primary }, (!text.trim() || isPosting) && styles.postButtonDisabled]}
          onPress={handlePost}
