  ...
  const renderMedia = () => {
    switch (option.mediaType) {
      case CrownMediaType.image:
        if (option.mediaUrl) {
          return <Image source={{ uri: option.mediaUrl }} style={styles.media} resizeMode="cover" />;
        }
        break;
      case CrownMediaType.video:
        ...
        return (
          <View style={[styles.media, styles.placeholderMedia]}>
            {option.mediaUrl && <Image source={{ uri: option.mediaUrl }} style={styles.media} resizeMode="cover" />}
          </View>
        );
      case CrownMediaType.audio:
        return (
          <View style={[styles.media, { backgroundColor: themeColor, opacity: 0.2 }]} />
        );
      case CrownMediaType.lottie:
        if (option.mediaUrl && LottieView) {
          ...
          return (
            <View style={[styles.media, { justifyContent: 'center', alignItems: 'center', backgroundColor: '#E5E5E5' }]}>
              <LottieView
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        ...
        { width, height },
        option.isMe && { borderColor: themeColor, borderWidth: 2 }
      ]}
    >
...
const styles = StyleSheet.create({
  cardContainer: {
    borderRadius: 16,
    backgroundColor: '#E5E5E5', // surfaceContainerHighest fallback
    borderWidth: 2,
