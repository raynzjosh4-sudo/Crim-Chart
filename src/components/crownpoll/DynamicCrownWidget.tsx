...
export const DynamicCrownWidget: React.FC<DynamicCrownWidgetProps> = ({
  ...
  const styles = useStyles(colors => ({
    ...
    scrollContent: {
      ...
      // Re-add padding inside scroll view
      paddingBottom: 20,
    ...
    setOptions(prevOptions => {
      const newOptions = prevOptions.map(opt => {
        if (opt.isMe && opt.id !== optionId) {
          return opt.copyWith({
            crowns: Math.max(0, opt.crowns - 1),
            isMe: false
          });
        }
        if (opt.id === optionId) {
  ...
  return <View style={styles.container}>
      ...
      <View style={styles.titleContainer}>
        <ExpandableCrownText text={pollModel.title} style={styles.titleText} />
      </View>

      {/* THE HORIZONTAL CARDS */}
      <View style={[styles.listContainer, fullBleed && styles.fullBleed]}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          {options.map(option => <View key={option.id} style={styles.cardWrapper}>
              <CrownPollCard option={option} themeColor={themeColor} onTap={() => {
          ...
          <TouchableOpacity activeOpacity={1} style={[styles.addCard, {
          backgroundColor: `${themeColor}1A`,
          // 0.1 alpha
          borderColor: `${themeColor}4D` // 0.3 alpha
        }]} onPress={() => setIsAddingOption(true)} activeOpacity={0.8}>
      ...
      {isAddingOption && <View style={styles.inputRow}>
      ...
      }]} value={newOptionText} onChangeText={setNewOptionText} placeholder="Add your option text..." placeholderTextColor="#999" autoFocus onSubmitEditing={submitNewOption} />
