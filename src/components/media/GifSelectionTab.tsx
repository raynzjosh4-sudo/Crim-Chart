...
const DUMMY_GIFS = Array.from({ length: 6 }, (_, i) => ({
  id: i,
  url: `https://picsum.photos/seed/gif${i}/400/400`,
}));

export default function GifSelectionTab({ selectedIndices, onMediaTap }: GifSelectionTabProps) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.searchRow}>
        {isSearchExpanded ? (
          <TextInput
            style={[styles.searchInput, { color: colors.text, backgroundColor: 'rgba(255,255,255,0.05)' }]}
      ...
      {isCategoriesExpanded && (
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={[styles.categoryChip, { backgroundColor: 'rgba(255,255,255,0.08)' }]}>
              <Text style={[styles.categoryChipText, { color: colors.text }]}>{item}</Text>
            </View>
          )}
        />
      )}
