...
export const CommentItem: React.FC<CommentItemProps> = ({ comment, onReply, onLike }) => {
  ...
  return (
    <View style={styles.container}>
      <ExpoImage
        source={comment.author_avatar_url ? { uri: comment.author_avatar_url } : require('../../../assets/images/favicon.png')}
        style={styles.avatar}
        contentFit="cover"
      />
      ...
      <View style={styles.likeContainer}>
        ...
        >
          <Heart
            size={16}
            color={isLiked ? colors.primary : "rgba(255,255,255,0.4)"}
            fill={isLiked ? colors.primary : "transparent"}
          />
        </TouchableOpacity>
        {likesCount > 0 && (
          <Text style={[styles.likeCount, isLiked && { color: colors.primary }]}>{likesCount}</Text>
        )}
      </View>
    </View>
  );
