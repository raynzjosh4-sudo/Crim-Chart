...
import { Modal, Platform, Pressable, SafeAreaView, ScrollView, StyleSheet, Text, useWindowDimensions, View } from 'react-native';
            ...
            backgroundColor: 'rgba(236, 236, 236, 0.2)'
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                        ...
                        <View style={styles.headerRow}>
                            <AppAvatar imageUrl={userProfileImageUrl} size={32} hasStatus isOnline={false} onTap={() => {
              onClose();
              ...
              uri: statusImageUrl
                            ...
                            <View style={styles.toggleContainer}>
                                <Pressable onPress={() => setShowChannels(!showChannels)} style={({
                pressed
              }) => [styles.toggleButton, {
                backgroundColor: showChannels ? `${themeColors.primary}1A` // Hex code append with 10% Alpha
                : 'rgba(28, 25, 25, 0.3)',
                opacity: pressed ? 0.8 : 1
              }]}>
                                    <MaterialIcons name={showChannels ? 'cached' : 'sync'} size={26} color={showChannels ? themeColors.primary : 'rgba(236, 236, 236, 0.6)'} />
                                </Pressable>
                            </View>
                        </View>

                        {/* Conditional Channel Selector Block */}
                        {showChannels && <View style={styles.channelWrapper}>
                                <ChannelSelectorRow title={t('select_channel_to_Chart')} selectedIndex={selectedChannelIndex} onChannelSelect={(index: number) => setSelectedChannelIndex(index)} onChartConfirm={(index: number) => {
              onClose();
              ...
              // TODO: Implement report post logic
                            ...
                            {renderActionItem(t('report_user', {
              username
            }), true, () => {
              onClose();
            }, themeColors)}
...
function renderActionItem(title: string, isDestructive: boolean, onPress: () => void, themeColors: any) {
  return <View style={styles.actionPadding}>
      ...
      opacity: pressed ? 0.8 : 1
                ...
                <MaterialIcons name={isDestructive ? 'report-problem' : 'person'} size={18} color={isDestructive ? '#FF5252' : 'rgba(236, 236, 236, 0.6)'} />
        ...
        color: isDestructive ? '#FF5252' : 'rgba(236, 236, 236, 0.9)'
                ...
                <MaterialIcons name="chevron-right" size={16} color="rgba(236, 236, 236, 0.3)" />
            </Pressable>
        </View>;
}
