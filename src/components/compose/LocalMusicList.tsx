...
export const LocalMusicList: React.FC<LocalMusicListProps> = ({ onSelect, selectedId, prefetchedAssets }) => {
  ...
  if (!prefetchedAssets && !permissionResponse?.granted) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ color: theme.colors.text }}>Permission needed to access local music.</Text>
        <TouchableOpacity
          style={{ marginTop: 12, padding: 12, backgroundColor: theme.colors.primary, borderRadius: 8 }}
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={[styles.itemContainer, { borderBottomColor: 'rgba(255,255,255,0.1)' }]}>
            <AudioPreviewWidget
              media={{
                id: item.id,
                uri: item.uri,
        ...
        ListEmptyComponent={
          <View style={{ padding: 20, alignItems: 'center' }}>
            <Text style={{ color: theme.colors.text + '80' }}>No audio files found on device.</Text>
          </View>
        }
      />
    </View>
