  ...
  return <View style={{
    ...
    justifyContent: 'center'
      ...
      <TouchableOpacity activeOpacity={0.8} onPress={handlePress} style={styles.container} ref={triggerRef}>
        {children}
        {viewsCount > 0 && <View style={styles.badgeContainer}>
            <Text style={styles.badgeText}>{viewsCount}</Text>
          </View>}
      </TouchableOpacity>

      <StatusViewsMenuDialog visible={menuVisible} onClose={() => setMenuVisible(false)} anchor={menuAnchor} widgetSize={50} onViewersPress={handleViewers} onProfilePress={handleProfile} />
    </View>;
};