...
export default function MobileNumberPage() {
  ...
  const { pendingSignUp, startSignUp, setEmail: setStoreEmail, status } = useAuthStore();
      ...
      <View style={isDesktop ? styles.desktopWrapper : styles.flexOne}>
        <View style={[styles.content, isDesktop && styles.desktopModal]}>
          ...
          <TouchableOpacity activeOpacity={1}
            style={[styles.nextButton, (!email.includes('@') || email.length < 5) && styles.nextButtonDisabled]}
            onPress={handleNext}
            disabled={!email.includes('@') || email.length < 5 || isLoading}
          >
