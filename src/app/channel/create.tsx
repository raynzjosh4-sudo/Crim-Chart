...
export default function CreateChannelPage({
  ...
  return <View style={[styles.root, {
    backgroundColor: isDesktop ? 'rgba(0,0,0,0.85)' : colors.background
  }, isDesktop && styles.desktopBackdrop]}>
      <View style={[styles.container, {
      backgroundColor: colors.background
    }, isDesktop && styles.desktopWindow]}>
      <ChartAppBar title={tr('create_channel')} showBack={true} centerTitle={true} titleStyle={{
        fontWeight: '900',
        textTransform: 'lowercase'
      }} onBack={isInline ? () => router.setParams({
        desktopChannelId: ''
      }) : undefined} />
      ...
      <ScrollView style={{
        ...
        <View style={styles.avatarContainer}>
          <TouchableOpacity activeOpacity={0.8} onPress={handlePickImage}>
            <View style={[styles.avatarCircle, {
              ...
              {selectedMedia ? <ExpoImage source={{
                uri: selectedMedia.contentUrl
              }} style={styles.avatarImage} contentFit="cover" /> : <Camera size={40} color="#FFD700" opacity={0.8} />}
            </View>
          </TouchableOpacity>
        </View>
        ...
        <TextInput style={[styles.input, {
          ...
          outlineStyle: 'none'
        } as any : {}]} placeholder={tr('channel_title_hint')} placeholderTextColor="rgba(255,255,255,0.3)" value={title} onChangeText={setTitle} />
        <TextInput style={[styles.input, styles.textArea, {
          color: colors.text,
          backgroundColor: 'rgba(255,255,255,0.05)'
        }, Platform.OS === 'web' ? {
          outlineStyle: 'none'
        } as any : {}]} placeholder={tr('channel_description_hint')} placeholderTextColor="rgba(255,255,255,0.3)" multiline numberOfLines={4} value={description} onChangeText={setDescription} />
        ...
        <View style={styles.createButtonContainer}>
          <TouchableOpacity onPress={handleCreate} activeOpacity={0.7} disabled={isLoading}>
            <View style={[styles.createButton, isLoading ? styles.createButtonLoading : styles.createButtonActive]}>
              <Text style={[styles.createButtonText, isLoading ? {
                color: 'rgba(250, 205, 17, 0.5)'
              } : {}]}>
                {tr('create')}
              </Text>
            </View>
...
const ActionTile = ({
  ...
  styles
    ...
    <View style={styles.tileTrailing}>
      ...
      <ChevronRight size={20} color="rgba(255,255,255,0.4)" style={{
      marginLeft: 8
    }} />
    </View>
  </TouchableOpacity>;
const ToggleTile = ({
  ...
  styles
    ...
    <Switch value={value} onValueChange={onChanged} trackColor={{
    false: 'rgba(255,255,255,0.1)',
    true: 'rgba(250, 205, 17, 0.5)'
  }} thumbColor={value ? '#FACD11' : '#f4f3f4'} style={{
    transform: [{
      scale: 0.8
    }]
  }} />
